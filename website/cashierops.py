...
def get_product_names(productType):
    try:
        products = prodInventory.query.filter_by(ptype=productType, status='Available').all()
...
def generate_pdf():
    ...
    subTitle2 = f"Transaction Generated by {current_user.first_name} {current_user.last_name} ({current_user.eid})"
    transactionno = f"Receipt No. {transaction_id}"
    dateoftransaction = f"Date: {currentdate}"
    clientname = f"Client Name: {client_name}"
    buffer = BytesIO()
    ...
    pdf.drawString(72, 735, transactionno)
    pdf.drawRightString(pagewidth - 40, 735, dateoftransaction)
    pdf.drawString(72, 725, clientname)
    pdf.setFont('Helvetica', 10)
    pdf.drawString(50, 20, subTitle)
    pdf.drawRightString(pagewidth - 40, 20, subTitle2)
    ...
    pdf.drawString(360, y_position - len(data) * 20 - 20, f"Total Amount:          P{total_amount:.2f}")
    pdf.drawString(360, y_position - len(data) * 20 - 40, f"Cash Tendered:       P{cash_tendered:.2f}")
    
    change = cash_tendered - total_amount
    pdf.drawString(360, y_position - len(data) * 20 - 60, f"Change:                    P{change:.2f}")
